  ...
  var warryManager = {
    ...
    _WarryCustom: function () {
      ...
      vtexjs.catalog.getCurrentProductWithVariations().done(function (product) {
        const productId = product.skus[0].skud;
            ...
            function add_items_offerings() {
              $(".warranty .skuService").before(
                "" +
                  ...
                  '<div id="warry-shop" data-skuid=' +
                  productId +
                  ">Agregar garantia extendida</div>" +
                  '<div class="box-warry">' +
              ...
              setTimeout(() => {
                $(
                  ".warranty .warry-togle-nav .item-warry-shelf input[type=radio]"
                ).click(function () {
                  const valueservice = $(this).val();
                  console.log(valueservice);

                  $("body").on("click", "#warry-shop", function (e) {
                    e.preventDefault();
                    const that = $(this);
                    const skuId = that.attr("data-skuid");
                    if (skuId != "") {
                      $.ajax({
                        url:
